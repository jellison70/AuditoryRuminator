...
class Test6ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) {
            ...
            let url = Bundle.main.url(forResource: "warble3k_pulse_ramp_0p45sX2_24bit", withExtension: "wav")
    ...
    func adaptation() {
        if initial == true {
            level = IntroViewController.GlobalVar.calLevelL[2]
            ...
            if upReversal == 2 && downReversal >= 2 {
                ...
                let newViewController = self.storyboard?.instantiateViewController(withIdentifier: "Test7ViewController")
            ...
            audioPlayer.setVolume(Float(level), fadeDuration: 0)
            audioPlayer.play()
            if round(level / IntroViewController.GlobalVar.calLevelL[2])  >= 10.0 {
                print("Maximum level Achieved: \(level), ratio = \(level / IntroViewController.GlobalVar.calLevelL[2])")
                cntMax += 1
    ...
    func showCancelAlert() {
        ...
        let cancelAction2 = UIAlertAction(title: "Continue", style: .default) { (cancelAction2) -> Void in
            let viewControllerNo = self.storyboard?.instantiateViewController(withIdentifier: "Test6ViewController")
