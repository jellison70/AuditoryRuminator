...
class Test2ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) {
            ...
            do {
                ...
                self.self.audioPlayer.pan = 1.0
                self.audioPlayer.setVolume(Float(self.level), fadeDuration: 0)
                self.audioPlayer.play()
                print("upReversal: \(self.upReversal), downReversal: \(self.downReversal), level: \(self.level), audible: \(self.self.isAudible)")
    ...
    func adaptation() {
        if initial == true {
            ...
            if upReversal == 2 && downReversal >= 2 {
                ...
                let newViewController = self.storyboard?.instantiateViewController(withIdentifier: "Test3ViewController")
    ...
    func showCancelAlert() {
        ...
        let cancelAction2 = UIAlertAction(title: "Continue", style: .default) { (cancelAction2) -> Void in
            let viewControllerNo = self.storyboard?.instantiateViewController(withIdentifier: "Test2ViewController")
