...
class Test6ViewController: UIViewController {
    ...
    func adaptation() {
        if initial == true {
            level = TestViewController.GlobalVar.calLevelL[2]
            ...
            audioPlayer.setVolume(Float(level), fadeDuration: 0)
            audioPlayer.play()
            if round(level / TestViewController.GlobalVar.calLevelL[2])  >= 10.0 {
                print("Maximum level Achieved: \(level), ratio = \(level / TestViewController.GlobalVar.calLevelL[2])")
                cntMax += 1
