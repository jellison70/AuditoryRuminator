...
class Test3ViewController: UIViewController {
    ...
    func adaptation() {
        if initial == true {
            level = IntroViewController.GlobalVar.calLevelR[3]
            ...
            if upReversal == 2 && downReversal >= 2 {
                ...
                let newViewController = self.storyboard?.instantiateViewController(withIdentifier: "Test4ViewController")
            ...
            audioPlayer.setVolume(Float(level), fadeDuration: 0)
            audioPlayer.play()
            if round(level / IntroViewController.GlobalVar.calLevelR[3])  >= 10.0 {
                print("Maximum level Achieved: \(level), ratio = \(level / IntroViewController.GlobalVar.calLevelR[3])")
                cntMax += 1
    ...
    func showCancelAlert() {
        ...
        let cancelAction2 = UIAlertAction(title: "Continue", style: .default) { (cancelAction2) -> Void in
            let viewControllerNo = self.storyboard?.instantiateViewController(withIdentifier: "Test3ViewController")
